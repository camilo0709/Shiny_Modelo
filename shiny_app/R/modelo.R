...
VARS_META <- list(
  ...
  GenHlth  = list(label = "Estado de salud general autopercibido",
                  type  = "slider",
                  min = 1, max = 5, step = 1,
                  marks = c("Excelente", "Muy bueno", "Bueno", "Regular", "Malo")),
  ...
  Age      = list(label = "Rango de edad",
                  type  = "slider",
                  min = 1, max = 13, step = 1,
                  marks = c("18-24", "25-29", "30-34", "35-39", "40-44",
...
# ── Helper: fila de etiquetas debajo del slider ────────────────────────────────
slider_marks_ui <- function(labels) {
  div(class = "slider-marks",
      lapply(labels, function(lbl) span(lbl))
  )
}
...
make_input_ui <- function(ns, var_id, meta) {
  ...
  input_ui <- if (meta$type == "radio") {
    ...
    # Slider con etiquetas personalizadas debajo
    tagList(
      sliderInput(
        inputId = ns(paste0("input_", var_id)),
        label   = NULL,
        ...
        value   = meta$min,
        width = "100%",
        ticks   = FALSE       # ocultamos los números nativos de Shiny
      ),
      slider_marks_ui(meta$marks)
    )
  }
...
modeloUI <- function(id) {
  ...
  div(style = paste0("background-color:", BG_PAGE_M, "; padding:2rem 3rem; min-height:100vh;"),
    ...
    p(paste0(
      "Complete los campos clínicos y sociodemográficos para obtener una estimación ",
      "de la probabilidad de enfermedad cardíaca. El modelo fue entrenado con la ",
      "encuesta BRFSS 2015 (253,680 registros) empleando las 12 variables de mayor ",
      "relevancia predictiva (AUC = 0.84)."
    ),
...
modeloServer <- function(id, df_global) {
  moduleServer(id, function(input, output, session) {
    ...
    # Entrenar modelo (una sola vez, al iniciar la sesión)
    ...
    observeEvent(input$btn_predict, {
      ...
      # Recoger valores; los sliders siempre tienen valor (min por defecto),
      # solo los radio buttons pueden estar vacíos
      ...
      output$resultado_prediccion <- renderUI({
        div(
          ...
          # Nota metodológica
          p(paste0(
            "Nota metodológica: Esta herramienta tiene fines académicos e ilustrativos. ",
            "La probabilidad se estima a partir de un modelo XGBoost entrenado sobre datos ",
            "poblacionales de la encuesta BRFSS 2015 y no constituye un diagnóstico clínico."
          ),
          style = paste0("color:", TEXT_SEC_M, "; font-family:", FONT_M, ";",
